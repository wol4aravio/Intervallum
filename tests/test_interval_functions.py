...
from intervallum.inclusion_functions import RS_SubinclusionFunction


def test_base():
    def f(x: np.ndarray):
        return x[0] ** 2 - x[0] ** 2 + x[1] ** 2
    rs_f_10 = RS_SubinclusionFunction(f, number_of_samples=10)
    rs_f_100 = RS_SubinclusionFunction(f, number_of_samples=100)
    rs_f_10000 = RS_SubinclusionFunction(f, number_of_samples=10000)
